...
const symbols = ["(", ")", "{", "}", "[", "]", ",", ":", "?", "=>", "!", "&&", "||", "+", "-", "*", "/", "%", "<", ">", "<=", ">=", "==", "!=", "=", ";", "...", ".", "//", "let", "function", "return"]
...
export const tokenize = (code:string, i:number=0, tid = 0):token[] =>{
  ...
  comp("null") ? ["null", i+4]:
  comp("let") ? ["symbol", i+3]:
  comp("function") ? ["symbol", i + "function".length]:
  comp("return") ? ["symbol", i + "return".length]:
...
// const getChilds = (a:ast):ast[]=>{ return a.children}
// const appC = (fn:(a:ast)=>ast, a:ast):ast =>{return {...a, children:a.children.map(fn)} as ast}

const freeVars = (a:ast):string[]=>{
  if (a.type == "identifier") return [a.value]
  let vars = a.children.reduce((p,c)=>
    p.concat(freeVars(c).filter(v=>!p.includes(v)))
  , [] as string[])

  if (a.type == "=;"){
    vars = vars.filter(v=>!freeVars(a.children[0]).includes(v))
  }
  return vars
}
...
/*
stringify ast in polish notation
*/
const stringify = (ast:ast):string => {
  if (ast.type == "number" || ast.type == "boolean" || ast.type == "null" || ast.type == "identifier" || ast.type == "string") return ast.value
  if (ast.type == "typo") return "typo: "+ast.value
...
const test_parse = (code:string, expected:string) =>{
  const ast = getAst(tokenize(code))
  const res = stringify(ast)
  assertEq(res, expected, `parse error: ${res}\n${expected}`)
}
